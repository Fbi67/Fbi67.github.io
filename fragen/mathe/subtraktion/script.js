...
// Funktion, um eine neue Frage zu stellen (Subtraktion)
function FrageStellenSubtraktion() {
  if (runden == 10) {
...
var zahlen = FrageStellenSubtraktion();

// Funktion zur Prüfung der Antwort (Subtraktion)
function fragePruefenSubtraktion(zahl1, zahl2) {
  ...
  if (isNaN(antwort)) {
    ...
    dialog.setOnCloseCallback(() => {
      zahlen = FrageStellenSubtraktion();
    });
    Falsch++;
...
document.getElementById("pruefen").addEventListener("click", function () {
  fragePruefenSubtraktion(zahlen[0], zahlen[1]);
